...
class Store:
    ...
    def delete_events_older_than(self, cutoff: str, chunk: int = 5000) -> int:
        ...
        event_time - тот про время события У ИСТОЧНИКА: может отсутствовать или быть в
        прошлом/будущем при replay исторических датасетов, даже будучи канонизированным на
        записи (app/timeutil.py); ingested_at - наше собственное время приёма, всегда
