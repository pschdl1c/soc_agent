...
| GET | `/alerts` · `/alerts/{id}` | Список / карточка алерта. Список — обёртка `{alerts, total, limit, offset}` (как у `/events`·`/incidents`; голым массивом был до появления пейджера в UI — без `total` вкладка молча показывала только первую страницу). Фильтры: `source_batch`, `rule_level`, `time_from/to` (фильтра по `status` НЕТ — у алерта нет статуса); сортировка `sort_by` (`rule`=severity/`host`/`event_count`/`created_at`) + `sort_dir`; пагинация проверяется общим `main._check_paging` (`limit` 1..500, `offset` >= 0 — иначе 400, как у `/events`). Карточка (`/alerts/{id}`) дополнительно несёт `mitre` — обогащение тегов через `app/kb.py` (`[{tag, technique_id, name?, url?, tactics?, matched}]`), список `/alerts` — нет. Ручки на смену статуса алерта нет — триаж-статус только у инцидентов. |
...
### Этап 4.5 — Детект-контент 🎯 ТЕКУЩАЯ ИТЕРАЦИЯ 🟡
...
- **Ссылки `correlation.rules` проверяются на СОХРАНЕНИИ и резолвятся по ВСЕМ custom-рулсетам.**
  ...
  на которые ссылаются активные корреляции (строки с `via_dependency=True`) — и для main, и для
  `ruleset=custom_rulesets/X` (единая точка `main_ruleset.resolve_for`). Удаление правила/рулсета
  и смена `name`, на которые ссылаются другие корреляции — `409` (`CatalogConflict`), `force=true`
  удаляет всё равно. `member_rules` в `/incidents/{id}/context` ищутся по title во всех рулсетах.
...
|---|---|---:|---|
| 4688 / Sysmon 1 | да, обильно | 1349 / 1350 | ядро process-контента |
| Sysmon 13 / 11 | да, обильно | 261 / 211 | 80% объёма 13 — `CompatTelRunner.exe` (Amcache), резать в конфиге Sysmon |
