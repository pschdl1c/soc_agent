...
| `store.py` | SQLite-хранилище. Две логически независимые таблицы: `alerts` и `events`. Дедуп алертов по `dedup_key`. **Два соединения с раздельными локами**: `_conn`/`_lock` — только запись, `_read_conn`/`_read_lock` (с `PRAGMA query_only=ON`) — только чтение; БД в `PRAGMA journal_mode=WAL` (+`synchronous=NORMAL`) — читатели (просмотр/фильтр/группировка Событий) не блокируют писателя (ingest-воркер) и наоборот. Индекс на выражении (`idx_events_json_eventid`) для «горячих» JSON-полей (см. `filter_lang.INDEXED_JSON_FIELDS`) — иначе фильтр/группировка по кастомному полю сканируют всю таблицу. `health()` — для `/health` (`SELECT 1` через `_conn`, опционально счётчики строк/размер БД). `delete_batch(source_batch)` — удаляет из обеих таблиц разом (для `DELETE /batches/{source_batch}`). |
| `normalize.py` | `raw_results` Zircolite → список `Alert`. Один прогон правила разбивается на алерты по **(хосту, источнику)** — источник берётся из `INGEST_SOURCE_FIELD`, временно вписанного в событие перед прогоном движка (см. `ingest_queue.py`/`main.py`), с фолбэком на `default_source_batch`, если маркера нет (одноисточниковые `/ingest/file`·`/ingest/upload`). Извлекает `entities`, берёт sample-события (первые N + последние N). |
...
| `rules_catalog.py` | Каталог Sigma-рулсетов/правил для вкладки «Sigma-правила»: built-in (`Zircolite/rules/*.json`, read-only) и любое число ИМЕНОВАННЫХ custom-рулсетов (`custom_rulesets/<id>/` — `meta.json` + `*.yml` на правило, source of truth, + `.manifest.json` — кэш скомпилированных метаданных для быстрого браузинга). Просмотр builtin/custom — обычный `json.load()` с кэшем по mtime (без pySigma); компиляция через `zircolite.rules.RulesetHandler` нужна только при добавлении нового custom-правила/рулсета (`save_custom_rule`/`save_ruleset_yaml`, оба принимают `ruleset` — существующий, ИЛИ `new_ruleset_name` — создать новый; builtin как цель отклоняется). Ничего не знает про «основной рулсет» (main) — однонаправленная зависимость `main_ruleset.py → rules_catalog.py`. |
...
- ⬜ Корреляция алертов в **инциденты** (по хосту/пользователю/временному окну).
