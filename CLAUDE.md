...
- **`alerts`** — нормализованные алерты для аналитика и будущего агента. Дедуп — два режима
  ...
  `sample_events`, `status` — в БД остался (`new → investigating → closed`), но UI его больше
  не показывает и не меняет (ручки `PATCH /alerts/{id}/status` тоже нет — статус только у
  инцидентов, см. `incidents.status` ниже).
...
- **`incidents`** (Этап 4) — агрегат срабатываний, будущая единица работы агента. Заводится
  ...
  алерта). Дедуп по `dedup_key = sha256(incident_type:group_values:window_bucket)[:16]` —
  фиксированный бакет по `timespan` правила: повтор в бакете → UPDATE, разрыв больше `timespan`
  → новый инцидент. Поля: `incident_type`/`title`/`severity`/`status` (`new → investigating →
...
| GET | `/alerts` · `/alerts/{id}` | Список / карточка алерта. Фильтры: `source_batch`, `status`, `rule_level`, `time_from/to`; сортировка `sort_by` (`rule`=severity/`host`/`event_count`/`created_at`) + `sort_dir`. Карточка (`/alerts/{id}`) дополнительно несёт `mitre` — обогащение тегов через `app/kb.py` (`[{tag, technique_id, name?, url?, tactics?, matched}]`), список `/alerts` — нет. `status` в ответе есть (в БД не убирался), но ручки на смену статуса алерта больше нет (`PATCH /alerts/{id}/status` удалён) — триаж-статус теперь только у инцидентов. |
...
| GET | `/incidents/{id}/context` | Полный контекст под агента/триаж: `correlation_rule` (структурные поля), `member_rules` (`+ yaml_text`/SQL), `sample_events`, `related_events` (события по сущности через `compile_filter_query(_incident_entity_filter(group_key))`), `entity_history`. Удалённое правило / вычищенные ретеншном `events` → пустые секции + `note`, не 5xx. |
| PATCH | `/incidents/{id}/status` | Смена статуса инцидента (`new → investigating → closed`). |
...
- ✅ **Таблица `incidents`** — агрегат срабатываний, единица работы агента. Заводится НЕ
  ...
  повтор в бакете обновляет строку. Тип инцидента параметризуемый (`incident.type` — slug).
...
- ✅ **`GET /incidents/{id}/context`** — полный контекст одним ответом: correlation-правило + member-
  ...
  (`compile_filter_query(_incident_entity_filter(group_key))`), история. Переживает вычищенные
...
- **Инциденты (`app/detection/correlation.py` + `incidents`/`investigations`, Этап 4) - грабли:**
  ...
  - `incidents.dedup_key` - фиксированный бакет по `timespan` (`incident_type:group_values:
    window_bucket`). Повтор ключа в том же бакете = UPDATE строки (окно расширяется, severity
    пересчитывается), НЕ новый инцидент. Разрыв больше `timespan` = новый бакет = новый инцидент.
