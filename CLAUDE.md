...
- **Инциденты (`app/detection/correlation.py` + `incidents`/`investigations`, Этап 4) - грабли:**
  ...
  - `app/incidents.py:run_pending` - ЗАГЛУШКА Этапа 4 (ставит `verdict="needs-review"` без
    ...
    остаются.
  - Привязка member-алертов (`store.link_alerts_to_incident`) БЕЗ временнóго сужения по
    `created_at` (наивный wall-clock приёма ≠ `event_time` источника при replay) - алерт того
    же правила/сущности из соседнего окна может прилипнуть. `events` на этом пути не трогается
    (то же требование производительности, что у счёта корреляции).
