...
- 🟡 **Стейтфул-корреляция** Sigma-правил (`event_count`/`value_count`) поверх постоянной
  ...
  — ⬜ не эвалуируются; отложено за пределы MVP агента (первый тип инцидента — brute-force,
  покрывается существующим `event_count`). Агрегация алертов в **инциденты** (`incidents` —
  единица работы агента, склейка нескольких алертов) — ⬜, вынесена в Этап 4.
