...
> **Незакоммиченные изменения (Этап A, движок сценарной корреляции).** В рабочей копии лежит
> реализация Этапа A дорожной карты (§7) — переписанный `app/detection/correlation.py` (все
> четыре типа correlation-правил, двухфазный счёт независимо от размера БД, цепочки), ECS-lite
> колонки, ретеншн `events`, миграция схемы, тесты, обновлённая документация. **Не коммитить
> без явного запроса пользователя** — изменения ожидают проверки на живом сервере (реальный
> `/ingest/stream`, не только `Store`/`correlation` напрямую в тестах). Это не общее правило
> "никогда не коммитить" — обычные инструкции по коммитам (см. системный промпт: коммитить
> только по прямой просьбе) продолжают действовать как обычно; эта заметка — про КОНКРЕТНО эти
> изменения, снять её можно после того, как пользователь подтвердит коммит или прогон.
...
Тесты: `pip install -e ".[dev]" && pytest` (см. `tests/`, конфиг — `pyproject.toml`
...
| DELETE | `/batches/{source_batch}` | Удаление источника-метки целиком: все его `events` И `alerts` (плюс `rule_hits`). `404`, если ничего не удалено. Регистрацию в `sources` (если есть) не трогает. |
| GET | `/alerts` · `/alerts/{id}` | Список / карточка алерта. Фильтры: `source_batch`, `status`, `rule_level`, `time_from/to`; сортировка `sort_by` (`rule`=severity/`host`/`event_count`/`status`/`created_at`) + `sort_dir`. Карточка (`/alerts/{id}`) дополнительно несёт `mitre` — обогащение тегов через `app/kb.py` (`[{tag, technique_id, name?, url?, tactics?, matched}]`), список `/alerts` — нет. |
...
- **`scripts/`** — вспомогательные скрипты для ручного тестирования (не часть приложения).
  ...
  - `bench_correlation.py` (Этап A) — бенчмарк масштабируемости коррелятора, только stdlib +
    ...
    внутри окна и замеряет `store.evaluate_correlation_windows` — прямая проверка требования
...
### Этап 4 — Инциденты и подготовка контекста для агента ⬜
- **Таблица `incidents`** — агрегат алертов, ЕДИНИЦА РАБОТЫ АГЕНТА (не сырое срабатывание
  правила: N алертов → M инцидентов, M ≪ N). Проход агрегации в коде по образцу
  `correlation.evaluate_batch` (не Sigma-правилом — нужно видеть поперёк всех рулсетов и обоих
  движков `zircolite`/`correlation`): группировка алертов по `(host|entity, скользящее окно)`,
  roll-up severity, агрегированные MITRE тактики/техники (покрытие kill-chain), ссылки на
  `alert_id`, статус. Тип инцидента параметризуемый; первый — brute-force (существующий
  `event_count`-алерт по 4625 → инцидент).
- **Хранилище вердиктов** — таблица `investigations`: `incident_id`, шаги расследования, вывод
  (`TP`/`FP`/`needs-review`), обоснование, confidence, `status` (`queued → running → done`),
  timestamps. Заполняется фоновой джобой (LLM медленный — синхронно на создании инцидента нельзя).
- **`GET /incidents/{id}/context`** — полный контекст одним ответом: правила инцидента +
  Sigma-логика (SQL/YAML), sample-события, связанные события по entity, история по хосту/
  пользователю. Переиспользует `store.list_events` + `app/filter_lang.py`.
- **Инструменты (tools)** — stack-agnostic функции-обёртки над `store`/`kb`/`filter_lang`:
  `search_events`, `get_incident_context`, `pivot_by_entity`, `lookup_mitre` (уже готов внутри
  `app/kb.py:get_technique`/`enrich_techniques` — осталось обернуть), (позже) threat-intel.
...
- **Счёт корреляции (`app/detection/correlation.py`) должен оставаться независимым от размера БД —
  это не пожелание, а проверяемое требование.** `store.evaluate_correlation_window(s)` считают
  ИСКЛЮЧИТЕЛЬНО по `rule_hits.group_json` (денормализованные значения полей, записанные ПРЯМО
  в леджер на `store_events`/`insert_correlation_hits`), БЕЗ `JOIN` к `events` — не возвращай
  `JOIN`/`json_extract(raw_json, ...)` на счётном пути, даже «ради простоты», это снова сделает
  скорость коррелятора зависимой от размера `events`. Двухфазный счёт (фаза 1 — один `GROUP
  BY`-запрос по объединённому окну сразу, фаза 2 — точная перепроверка только кандидатов) —
  тоже часть требования, не оптимизация про запас: наивный запрос-на-каждый-ключ снова даёт
  `O(K×H)`. Проверяй `scripts/bench_correlation.py` при любой правке счётного пути — рост
  `rule_hits` в 100 раз должен давать единицы процентов роста времени, не пропорциональный.
