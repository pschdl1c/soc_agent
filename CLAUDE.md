...
| `updates.py` | Лист-модуль без импортов проекта: счётчики изменений списков для автообновления UI (вкладки «Алерты»/«Инциденты» подтягивают новые строки без перезагрузки страницы). Состояние ТОЛЬКО в памяти процесса: `{epoch, alerts: {version, created}, incidents: {...}}`, отдаётся ручкой `GET /updates` (без похода в БД — её опрашивает каждая открытая вкладка раз в 7с). ДВА счётчика на канал не случайно: `version` растёт на ЛЮБОМ изменении (в т.ч. инкремент `event_count` у существующего алерта при дедупе, вердикт расследования, `DELETE /batches`), `created` — только на реально созданных строках; по «максимальному `created_at`» первое не видно вовсе (у `alerts` нет `updated_at`), и UI показывал бы устаревший список. `bump()` зовётся В ТОЧКАХ ЗАПИСИ (`main.py`, `detection/correlation.py`, `incidents.py`), а не в одной точке оркестрации — писателей трое. Работает, пока процесс ОДИН (uvicorn без `--workers`, см. §8). Подробно — `docs/spec/updates.md`. |
...
- **`incidents`** (Этап 4) — агрегат срабатываний, будущая единица работы агента. Заводится
  ...
  событиях дают два инцидента с общими алертами. Привязан к одному `source_batch` → чистится `delete_batch`. Переживает
...
| GET | `/updates` | Счётчики изменений списков для автообновления UI (`app/updates.py`): `{epoch, alerts: {version, created}, incidents: {version, created}}`. Считается в памяти, БЕЗ обращения к БД — ручку опрашивает каждая открытая вкладка раз в 7с, она обязана быть дешевле самого списка. Отдельно от `/health` (разный ритм опроса и разная семантика: свежесть данных против живости сервиса). |
...
| GET | `/alerts` · `/alerts/{id}` | Список / карточка алерта. Список — обёртка `{alerts, total, limit, offset}` (как у `/events`·`/incidents`; голым массивом был до появления пейджера в UI — без `total` вкладка молча показывала только первую страницу). Фильтры: `source_batch`, `rule_level`, `time_from/to`, `incident` (`all`\|`in`\|`none` — участие в инцидентах через `incident_alerts`), `q` (подстрока по правилу/хосту/значениям `entities`, регистронезависимо и для кириллицы — SQL-функция `py_lower`), `rule_title` (точное, раскрытие группы) — все собираются ОДНИМ `store._alerts_where` для списка, `total` и групп (фильтра по `status` НЕТ — у алерта нет статуса); каждая строка несёт `incident_ids`; сортировка `sort_by` (`rule`=severity/`host`/`event_count`/`created_at`) + `sort_dir`; пагинация проверяется общим `main._check_paging` (`limit` 1..500, `offset` >= 0 — иначе 400, как у `/events`). Карточка (`/alerts/{id}`) дополнительно несёт `incidents` (`[{incident_id, correlation_rule_title, severity, status, created_at}]` — блок «Входит в инциденты») и `mitre` — обогащение тегов через `app/kb.py` (`[{tag, technique_id, name?, url?, tactics?, matched}]`), список `/alerts` — нет. Ручки на смену статуса алерта нет — триаж-статус только у инцидентов. |
| GET | `/alerts/groups` | Режим «Группировать по правилу» вкладки «Алерты» — ОТОБРАЖЕНИЕ, не дедуп: `{groups: [{rule_title, rule_level (макс.), alert_count, event_count, in_incident_count, last_created_at}], total, limit, offset}`, фильтры как у `/alerts`, `sort_by` = `rule`\|`alert_count`\|`event_count`\|`created_at`. Объявлена до `/alerts/{id}`. Дедуп алертов по «служебным» полям (GUID/PID/порты) намеренно НЕ делаем — шум одинаковых алертов решается этой группировкой, фильтром `incident` и поиском. |
| GET | `/incidents` · `/incidents/{id}` | **Инциденты (Этап 4).** Список — обёртка `{incidents, total, limit, offset}`, фильтры `status`/`incident_type`/`source_batch`/`severity`/`time_from/to`, сортировка `sort_by` (`created_at`\|`updated_at`\|`alert_count`\|`status`\|`severity`)/`sort_dir`, `limit` 1..500 (общий `_check_paging`, вне диапазона — 400, раньше значение молча зажималось); каждая строка несёт `investigation_status`. Карточка — `+ member_alerts`, `investigation`, `mitre` (обогащение тегов правила + member-алертов). |
...
| DELETE | `/rules/custom/{rule_id}` | Удаление своего правила (обязательный query-параметр `ruleset` — из какого именованного custom-рулсета). Чистит точечную ссылку на правило в основном рулсете (`main_ruleset.on_rule_deleted`) — парно к `on_ruleset_deleted` у `DELETE /rulesets`. |
...
- **`data/custom_rulesets/`** (`SIEM_CUSTOM_RULESETS_DIR`; создаётся автоматически, сюда попадают только
  пользовательские данные из вкладки «Sigma-правила» и деплоя контента, `Zircolite/` не трогаем). В API
  рулсет адресуется логическим путём `custom_rulesets/<ruleset_id>` (`CUSTOM_PREFIX`) независимо от того, где
  каталог лежит на диске:
...
- **`artifacts/content/`** — детект-контент в git, источник правды (в SIEM попадает только через
  `scripts/deploy_content.py`): `<domain>/rules/*.yml` (базовое правило на файл), `<domain>/correlations/*.yml`,
  `<domain>/tests/<SCE_имя>.yml` (фикстуры `scripts/test_content.py`), `value_lists/<имя>.yml`, `telemetry/`
  (конфиг Sysmon стенда и схема полей событий для синтетики — в SIEM не грузятся). Имя каталога домена =
  имя custom-рулсета. Соглашения — §9.
...
- Пишешь не код, а детект-КОНТЕНТ (Sigma-правила, корреляции, листы значений) — §9, отдельный
  свод правил; §8 про движок, §9 про то, что в него кладут.
...
- 🟡 **Живое обновление списков** — ✅ Алерты и Инциденты подтягиваются без перезагрузки страницы:
  UI поллит дешёвую ручку `GET /updates` (счётчики версий в памяти процесса, `app/updates.py`), а не
  сам список; если обновить можно незаметно (активная вкладка, первая страница пейджера, карточка не
  открыта) — список перечитывается тихо, иначе показывается бейдж «↑ N новых алертов» и ждёт клика.
  События намеренно не покрыты (фильтр/группировка/drill-in — автоперезагрузка сломала бы разбор).
  ⬜ Осталось: push-транспорт (SSE/WebSocket) вместо поллинга — заменит только источник сигнала,
  счётчики и логика бейджа останутся; и общее (не in-process) состояние счётчиков, если приложение
  поедет на несколько воркеров. См. `docs/spec/updates.md`.
...
- ✅ **`ruleset` для `/ingest/stream`/`/ingest/events`** — оба теперь используют «основной
  ...
  main_ruleset.MAIN_RULESET_ID`, дальше `_process_batch` резолвит `main_ruleset.resolve_for()` в
  список правил (с межрулсетными зависимостями) и зовёт `engine.run_batch_with_rules(...)` (кэш
  `_rulesets_cache` тут не участвует — резолв и так дешёвый, читает уже скомпилированные правила через
  кэш `rules_catalog`). Состав основного рулсета собирается во вкладке «Sigma-правила» (кнопки на
...
`docs/guide/windows-vm-lab.md` (§3 — телеметрия и покрытие), агент — `docs/guide/windows-agent.md`.
...
- ✅ Движок, доводка 2026-09-15: NULL-колонки полей правил во флаше + лог ошибок SQL правил
  (`detection/engine.py`), member-алерты many-to-many (`incident_alerts`), резолв правил один раз на
  флаш; UI: фильтр «в инцидентах/вне», поиск и группировка по правилу во вкладке «Алерты», группировка
  алертов в карточке инцидента, бейдж «зависимость» и разбор 409 в «Sigma-правилах». Дедуп алертов по
  «служебным» полям намеренно не делаем — шум решается отображением.
- ✅ Телеметрия по эталонам (2026-09-15, §9): Sysmon — sysmon-modular balanced с ProcessCreate в
...
- **Контент правил — вынесен в Этап 4.5** (см. выше), пишется ДО агента: без настоящих
  инцидентов агента не на чем ни запускать, ни оценивать. Соглашения — §9.
...
- **Редактирование (`PUT /rules/custom/{rule_id}`) уже включённого в основной рулсет правила
  ...
  (`main_ruleset.json`), а `id` при редактировании остаётся исходным - другой
  ...
  `.manifest.json` (`_write_manifest` → `_invalidate_cache`), а `main_ruleset.resolve_for()`
...
- **Ссылки `correlation.rules` проверяются на СОХРАНЕНИИ и резолвятся по ВСЕМ custom-рулсетам.**
  Опечатка в ссылке — `400`, а не тихо сохранённое правило, которое `load_correlation_rules` потом
  пропускает. Индекс ссылок один на оба пути — `rules_catalog.build_ref_index` (глобальный, по
  `_scan_custom_rules`): новый источник имён правь там, иначе валидация и рантайм разъедутся.
  Цена глобального резолва — **глобальная уникальность `title` (ключ `rule_hits`) и `name` (ключ
  ссылки)** среди своих правил, проверяется на сохранении (`_check_global_uniqueness`; тот же `id` —
  это то же правило, коллизия по id, а не отказ пака). Исполнение подтягивает зависимости:
  `rules_catalog.with_dependencies` добавляет в прогон базовые правила/корреляции других рулсетов,
  на которые ссылаются активные корреляции (строки с `via_dependency=True` и `required_by` — какие
  корреляции потянули; в просмотре «Основного рулсета» UI показывает бейдж «зависимость» и не даёт
  тогглить такое правило) — и для main, и для
  `ruleset=custom_rulesets/X` (единая точка `main_ruleset.resolve_for`). Удаление правила/рулсета
  и смена `name`, на которые ссылаются другие корреляции — `409` (`CatalogConflict`, `detail = {message,
  references}`), `force=true` удаляет всё равно; UI (`deleteWithReferenceCheck`) показывает список
  ссылающихся корреляций и по второму подтверждению повторяет с `force=true`. `api()` в UI кладёт в
  ошибку `status`/`detail` — объектный `detail` иначе превращался в `[object Object]`. `member_rules` в `/incidents/{id}/context` ищутся по title во всех рулсетах.
- **Скан файлов правил кэшируется с перепроверкой раз в 2 секунды** (`_SCAN_RECHECK_SECONDS`), запись
  через `rules_catalog` сбрасывает кэш сразу (`invalidate_scan_cache`). Не убирай паузу: резолв main
  зовётся на каждом источнике каждого флаша, и с полной перепроверкой сигнатуры на каждую корреляцию
  он стоил ~0.5 с (замер на 197 правилах) — флаш с десятком источников не укладывался в интервал.
  Файлы, правленые мимо API, подхватываются с этой задержкой.
...
- **Zircolite строит таблицу флаша из полей событий; поля, упомянутые правилами, но не пришедшие ни
  в одном событии флаша, движок досоздаёт колонками со значением NULL** (`detection/engine.py`:
  `RuleColumnIndex` + `_SiemCore.add_missing_rule_columns`). Раньше такое правило падало с «no such
  column» и молча не срабатывало — даже если редкое поле стояло под `OR`. Список колонок правила
  выясняется у САМОГО SQLite (`EXPLAIN` запроса на пустой таблице, колонки добавляются по одной до
  успешной подготовки), не регэкспом, и кэшируется по тексту SQL: ~1.9 с один раз на 4291 правило
  встроенного рулсета, дальше миллисекунды. Не заменяй на разбор SQL регэкспом — литералы с `=` и
  `LIKE`-шаблоны дают ложные колонки. Грабли: SQLite трактует НЕИЗВЕСТНЫЙ идентификатор в двойных
  кавычках как строковый литерал (ошибки нет), имена с пробелами pySigma пишет в обратных кавычках.
  Остаётся семантика NULL: `NOT (поле LIKE ...)` на отсутствующем поле — ложь, фильтр по полю,
  которого стек не доставляет (`Provider_Name` — агент шлёт `ProviderName`, `_` режется flatten-ом),
  гасит правило. Поэтому синтетика `scripts/test_content.py` по-прежнему дополняет события реальным
  набором полей стенда (`artifacts/content/telemetry/event_fields*.json`, выгрузка —
  `scripts/export_event_fields.py`).
...
- **Автообновление списков (`app/updates.py`) — не поллить сам список и не считать «новых» по
  `created_at`.** UI опрашивает `GET /updates` (счётчики в памяти, без БД), а не `/alerts` —
  список с фильтрами это `count` + выборка, гонять её каждые несколько секунд дороже, чем ручное
  «Обновить». И у канала ДВА счётчика: `version` (любое изменение) отдельно от `created` (новые
  строки) — повторное срабатывание правила инкрементит `event_count` СУЩЕСТВУЮЩЕГО алерта
  (дедуп по `dedup_key`), у `alerts` нет `updated_at`, и по «максимальному `created_at`» такое
  изменение не видно вообще. Число новых считается по `dedup_key` ДО записи
  (`store.get_alert_ids_by_dedup_keys`): `upsert_alerts` возвращает число ОБРАБОТАННЫХ строк, а
  не созданных (это же число уходит в `IngestResponse.alerts_created`), не «упрощай» обратно.
  Схема верна, пока процесс ОДИН (uvicorn без `--workers`): при нескольких воркерах у каждого
  свой счётчик — состояние тогда переезжает в общее место вместе с переходом на SSE.
...
- **Инциденты (`app/detection/correlation.py` + `incidents`/`investigations`, Этап 4) - грабли:**
  ...
  - `store.delete_batch` теперь чистит ШЕСТЬ таблиц (`events`/`alerts`/`rule_hits`/`incidents`/
    `investigations`/`incident_alerts`). Добавляешь ещё одну таблицу с `source_batch` - не забудь её сюда же.
  ...
  - Привязка member-алертов (`store.link_alerts_to_incident`) - цепочкой `event_id` (реальный
    ...
    сущности, напр. DNS `QueryName`/путь ключа реестра). Связи пишутся в `incident_alerts`
    (many-to-many) — не возвращай одну колонку `alerts.incident_id` с `incident_id IS NULL`: при двух
    сценариях на одних событиях второй инцидент оставался без member-алертов. См. `docs/spec/incidents.md`. БЕЗ
...
Раздел про НАПИСАНИЕ детект-контента, а не про код движка (тот — §8). Правила ниже выведены
эмпирически на живом стенде `win10-lab` (см. `docs/guide/windows-vm-lab.md`), ссылки на код
проверены.
...
`<domain>/correlations/*.yml`, `<domain>/tests/<SCE_имя>.yml` (фикстуры), `value_lists/<имя>.yml`,
`telemetry/` (конфиги стенда + схема полей событий). В SIEM — только через
`scripts/deploy_content.py` (value lists → базовые правила всех доменов → корреляции в
...
- Корреляция, взводящая инцидент: `title: SCE_<Domain>_<Scenario>`; threat hunting —
  `SCE_TH_<Domain>_<Scenario>`, `incident.severity: low`, `incident.type` с префиксом `th_`. Сквозные
  сценарии — `SCE_KC_*`. Остальные правила (базовые, промежуточные, агрегаторы) — обычные названия.
- Правило SigmaHQ без правок — авторы как есть. Доработанное (включая guard `Channel`/`EventID`) —
...
- Каждому правилу — guard `guard_logsource: {Channel, EventID}` и `condition: guard_logsource and (...)`
...
- `Provider_Name` не доезжает (агент шлёт `ProviderName`, `_` режется flatten-ом) — в правиле писать
  `ProviderName`, если провайдер реально сужает детект, иначе убирать (роль играет guard). Свой маппинг
  в конфиге Zircolite намеренно не заводим — лишний слой ради одного имени. Поля `Initiated`/`DestinationIsIpv6` — числа (`Initiated: 1`, не `'true'`): агент
...
- Поле, которого стек не доставляет, делает правило мёртвым молча (§8, «no such column») — сверять
  имена с `telemetry/event_fields.json`.
...
другой `title`, то есть лишний алерт и лишние строки `rule_hits` на то же событие (и глобальная
уникальность `title` это запрещает).

- «Всё в одном рулсете» — ПЛОХО: рулсет — единица включения в main, удаления и изолированного прогона
  (`ruleset=custom_rulesets/<id>`, зависимости подтягиваются сами).
...
`event_count gte: 1` поверх нескольких базовых правил с тем же `group-by` (`Host LOLBIN File Download`,
`Host Shadow Copy Deletion`). Они же гасят двойной счёт одного приёма, видимого в двух телеметриях
(удаление теневых копий в Sysmon 1 и в 4104 — один приём для `temporal gte: 2`). Агрегаторы тактик
(`Host <Tactic> Activity`, `[Computer]`) лежат в своих доменах и кормят `killchain`.
...
`correlation.active_hit_spec` собирает ОБЪЕДИНЕНИЕ полей всех корреляций, ссылающихся на
правило: `spec.setdefault(ref["title"], set()).update(fields)`. Одно базовое правило, на которое
ссылаются три корреляции с разным `group-by`, получит в `group_json` все их поля разом, а каждая
корреляция вытащит свой срез через `json_extract`. **Отдельное базовое правило под каждую
комбинацию group-by заводить НЕ НУЖНО** — это была главная ошибка тестового контента (54 события
× 6 правил = 324 строки леджера вместо 54 × 3 = 162). Одиннадцать корреляций поверх трёх базовых
правил — норма.

### Guard на мусорные значения ключа ОБЯЗАТЕЛЕН

`store.store_events` пишет в `group_json` всё, что не `None`
(`{f: str(event[f]) for f in fields if event.get(f) is not None}`), а
`store.fetch_correlation_hits` отбрасывает строку, только если поля НЕТ вовсе (докстринг:
«Строки с None в любом компоненте ключа отбрасываются»). Значения `'-'` и `''` — полноправные
значения ключа: у 4625 при локальном входе `IpAddress` равен именно `'-'`, а не отсутствует.

Поэтому базовое правило, чей `group-by` идёт по такому полю, ОБЯЗАНО отфильтровать мусор само —
иначе корреляция отрапортует «20 неудач с IP `-`», сложив в один бакет все локальные неудачные
входы машины. Канонический вид:
...
Отсечка `informational` снята намеренно (`detection/normalize.py`, докстринг модуля): сработки
базовых правил нужны и как алерты (видимость/отладка сценария), и как member-алерты инцидента.
Следствие: широкое базовое правило = мусор во вкладке «Алерты» + строки в леджере. Ловили на
стенде: `SELECT * FROM logs WHERE EventID=4624` без фильтров дал 71 алерт, из которых 71 — это
`LogonType=5` от `NT AUTHORITY\СИСТЕМА`, то есть SCM поднимает службы.
...
(`TargetUserName|endswith: '$'`). Проверено: провалы брутфорса приходят `LogonType=3`,
интерактивный вход пользователя — `LogonType=2`, оба в списке.

### `title` — это ключ в `rule_hits`, `name` — ключ ссылки

Ссылки `correlation.rules` резолвятся по Sigma `name`, а в леджер пишется `title`
(`rule_hits.rule_title`). Переименование `title` базового правила НЕ рвёт ссылку, но осиротит
накопленные строки леджера — корреляция ослепнет на свой `timespan` (для `1d` это сутки), и
молча. Поэтому: `title` базового правила считать стабильным идентификатором, `name` задавать
всегда и явно, менять — только вместе с осознанным принятием слепого окна.

### Прочее, что всплывёт

- `correlation.incident` — только на ПОСЛЕДНЕМ звене цепочки (подробнее в §8, там же про
  `dedup_key` инцидента); внутри домена помеченным должен быть ровно один файл на сценарий.
- `timespan` не длиннее `SIEM_EVENTS_RETENTION_DAYS` — иначе 400 на сохранении
  (`_validate_correlation_doc`).
- Value lists — ГЛОБАЛЬНОЕ плоское пространство имён (`data/value_lists/<name>.yml`), не
  привязаны к рулсету. Именовать с префиксом домена (`auth_service_accounts`, `recon_tools`),
  иначе два домена подерутся за имя.
- **Порядок миграции контента: сначала переставить ссылки в корреляциях, ПОТОМ удалять базовые
  ...
  неразрешимой ссылкой (тихий пропуск в рантайме остался страховкой для правок мимо API),
  правило будет видно в UI и никогда не сработает.

### Что реально доступно на стенде (ограничивает, какой контент вообще тестируем)

Стенд `win10-lab` на телеметрии 2026-09-15: агент Vector, `telemetry/sysmonconfig.xml` (ProcessCreate
в exclude-режиме), аудит по базовой линии Microsoft, каналы Security, System, Sysmon/Operational,
PowerShell/Operational, Defender, WMI-Activity, TaskScheduler, Bits-Client. **Замер простоя** (20 мин
без действий после установившегося фона, 2026-09-15): **~1500 событий/ч**, ни одного алерта. Доли:
Sysmon 13 — 27% (`svchost`, `TiWorker`), 4702 — 13% (задачи `UpdateOrchestrator`), Sysmon 11 — 12%,
4688 — 12%, Sysmon 7 — 11% (Defender `MpCmdRun`, OneDrive), Sysmon 1 — 8%, 4957 — 5% (правила брандмауэра
не применены), 4624 type 5 / 4672 SYSTEM / 4799 `VSSVC` — по 2–3%. После загрузки разовые всплески:
4907 от `TiWorker` (сотни), 4945/4957 при старте брандмауэра — на установившемся фоне их нет или мало.
Фильтрация шума при таком объёме не нужна. Цифра «правил» — сколько записей `rules_windows_merged.json`
...
10/17 доезжают, но FP-доводка правил на них — после живого прогона.
