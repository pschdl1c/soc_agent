...
app/value_lists.py, вкладка «Списки»). Скрипт ТОЛЬКО шлёт события через POST /ingest/stream
...
Как пользоваться:
    ...
    7. Проверка «живого» списка: поменяй значения recon_tools во вкладке «Списки» и Сохрани —
       правило пересоберётся сразу (см. app/value_lists.py), следующий прогон это отразит.
...
def build_events(run: str) -> list[dict]:
    ...
    (rule_id, host, entity) (см. app/normalize.py), поэтому без суффикса повторный запуск
