...
Как пользоваться:
    ...
    2. python scripts/stream_correlation_test.py
    ...
    3. python scripts/stream_correlation_test.py --negative
...
Оставляет за собой батч событий с меткой source, которую печатает - можно удалить вручную через
вкладку "Источник данных" (DELETE /batches/{source_batch}) при желании.
"""
...
def _post_stream(url: str, source: str, events: list[dict]) -> None:
    body = "\n".join(json.dumps(e, default=str) for e in events).encode("utf-8")
    req = urllib.request.Request(
        f"{url}/ingest/stream?{urllib.parse.urlencode({'source': source})}",
        data=body, method="POST", headers={"Content-Type": "application/x-ndjson"},
    )
    try:
        with urllib.request.urlopen(req, timeout=15) as resp:
...
def main() -> None:
    ...
    while time.monotonic() - t0 < 20.0:
        ...
        if any(a.get("rule_title") == TARGET_RULE_TITLE for a in alerts):
            break
        time.sleep(1.5)

    hit = [a for a in alerts if a.get("rule_title") == TARGET_RULE_TITLE]
    if args.negative:
        if hit:
