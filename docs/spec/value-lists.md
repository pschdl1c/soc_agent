...
`_normalize_values` — trim, отброс пустых, дедуп с сохранением порядка; ПУСТОЙ результат
(`values: []` или всё отброшено) → `ValueListError`: правило с плейсхолдером на пустой список
не компилируется, то есть создать такой список можно было, а пользоваться им — нет. Проверка
общая для `create_list`/`update_list`/импорта. `> _MAX_VALUES` →
`ValueListError`.
