...
3. Результат дополняется `rules_catalog.with_dependencies(pairs)` — межрулсетными зависимостями
   активных корреляций: базовые правила и корреляции ДРУГИХ рулсетов, на которые ссылаются
   `correlation.rules` (транзитивно). Подтянутые строки — копии с `via_dependency=True` и `required_by`
   (названия корреляций, которые их потянули); в main они явно не включены, но исполняются — иначе
   корреляция на базовом правиле другого рулсета была бы мертва.
...
### `resolve_for(ruleset_path) -> list[tuple[str, dict]]`

Единая точка «что реально исполняется» для `ruleset_path`, с настоящим рулсетом-источником каждого
правила: `"main"` → `resolve_with_sources()`; `custom_rulesets/*` → все правила рулсета +
`with_dependencies`; builtin/`None` → `[]` (их гоняет `engine.run_batch`, корреляций там нет).
Несуществующий custom-путь — `CatalogNotFound` из `load_rules`.

Зовётся ОДИН раз на флаш из `app/main.py:_process_batch`: те же пары идут в движок и в
`correlation.correlation_rules_from_pairs()` → `corr_rules` для `active_hit_spec`/`evaluate_batch`.
Резолвить заново внутри цикла по источникам флаша не нужно (было 3 + N раз).

### `resolve() -> list[dict]`

`[rule for _src, rule in resolve_with_sources()]` — плоский список правил main. Пайплайн ingest его больше
не зовёт (там `resolve_for`), остаётся как публичный помощник.
...
Зовётся из `DELETE /rules/custom/{rule_id}`. Без неё state копил осиротевшие id: резолв
...
- Резолв дёшев (читает скомпилированные правила через mtime-кэш `rules_catalog` и кэш скана с
  перепроверкой раз в 2 с), поэтому считается на каждый флаш без отдельного кэша — но один раз на флаш.
