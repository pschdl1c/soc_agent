...
| `TIME_FIELDS` | `SystemTime`, `TimeCreated`, `EventTime`, `@timestamp`, `timestamp`, `EventReceivedTime`, `UtcTime` (порядок = приоритет; `TimeCreated` — момент записи события в журнал (агент Vector: ISO UTC с микросекундами; прежний Fluent Bit: `ДАТА ВРЕМЯ ±ЧЧММ`), `EventTime` — время чтения журнала форвардером) |
