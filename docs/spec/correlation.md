...
1. Уровень `informational` без блока `correlation.incident` — ТИХОЕ звено: считается и пишет своё
   попадание в `rule_hits` (на него ссылаются цепочки и агрегаторы), но алерта не создаёт. Раньше
   такое правило пропускалось целиком, и ссылающаяся цепочка молча не срабатывала. Инцидентное
   правило любого уровня работает как обычно, severity инцидента берётся из `incident.severity`.
...
7. `_evaluate_correlation_rule(...)` — A3-оценка (см. выше), с типоспецифичной логикой:
   ...
   - `temporal_ordered` — тот же порог, ПЛЮС `store.fetch_correlation_hit_sequence(...)` +
     ...
     проверка порядка появления, которой нет ни у одного апстрим Sigma-бэкенда. Попадания с
     РАВНОЙ меткой времени — одна ступень, порядок внутри неё не проверяется (у Windows-событий
     на прежнем Fluent Bit точность `TimeCreated` была секундной, у агента Vector — микросекунды, но
     равные метки дают и другие источники; SQL отдаёт ничьи в произвольном порядке).
   - операторы `condition`: `gte`/`gt`/`lte`/`lt`/`eq`/`neq`.
   - активные правила (`_active_correlation_rules`) — из `main_ruleset.resolve_for(ruleset_path)`,
     то есть вместе с межрулсетными зависимостями (`rules_catalog.with_dependencies`); у ссылок
     `base_rule_refs` есть `ruleset_path` рулсета, где лежит правило.
   - `event_ids` сработавшего окна дополняются реальными `event_id` из развёрнутых синтетических
     попаданий предков: у тихого звена нет алерта, по которому `link_alerts_to_incident` нашла бы
     member-алерты.
