...
Подключение Windows-хоста одним скриптом (аудит, Sysmon, агент Vector) — в
[`docs/guide/windows-agent.md`](./docs/guide/windows-agent.md); тестовый стенд (Windows-ВМ в
VirtualBox: сеть, проверка детекта) — в [`docs/guide/windows-vm-lab.md`](./docs/guide/windows-vm-lab.md).
...
- **Correlation-правила** (`event_count`/`value_count`/`temporal`/`temporal_ordered`, включая
  цепочки — correlation ссылается на другую correlation) считаются собственным движком
  (`app/detection/correlation.py`) поверх постоянных таблиц `events`/`rule_hits`, а не движком
  Zircolite — окно (`timespan`) реальное, не ограничено размером ingest-батча, и скорость
  счёта не зависит от размера БД (двухфазный счёт по `rule_hits.group_json`, без `JOIN` к
  `events`, см. `docs/spec/correlation.md`). Пишутся отдельным расширением файла (`.sigmacorr`),
  не `.yml`.
...
| `bench_correlation.py` | Бенчмарк масштабируемости коррелятора — без HTTP, только `app.store` (см. CLAUDE.md §5) |
...
| 1. Архитектура SIEM | 🟡 | Ingest → детект → хранение готовы; стейтфул-корреляция (`event_count`/`value_count`/`temporal`/`temporal_ordered`, включая цепочки) готова и не зависит от размера БД; ретеншн `events` готов; впереди — точная привязка entity, MITRE-обогащение, инциденты |
...
| 4. Инциденты и контекст для агента | ⬜ | Таблица `incidents` (агрегат алертов = единица работы агента), `investigations` (вердикты), API контекста инцидента, инструменты для агента |
| 5. **AI-агент расследования** 🎯 | ⬜ | Агент на LangGraph, «мозги» — OpenAI-совместимый API (провайдер/модель из `.env`). Tool-use цикл: гипотеза → вердикт TP/FP/needs-review. Оценка — живой контур с pentest-agent на ВМ |
